...
import MessageThreads from '../../api/collections/threads.js'
import { Log } from 'meteor/mozfet:meteor-logs'
export const name = 'server-startup-messages'
...
Meteor.publish('messages', function(threadId) {
  ...
  console.log('publish messages threadId:', threadId)
  const messageThread = MessageThreads.findOne(threadId)
  console.log('publish messages messageThread:', messageThread)
...
Messages.before.insert(function (userId, doc) {
  ...
  if (userId) {
    doc.senderId = userId
    doc.senderName = Access.userFirstName(userId) + ' ' +
        Access.userLastName(userId)
  }
  ...
  if (!doc.threadId) {
    ...
    // if no reciever
    if (!doc.recieverId) {
      throw new Meteor.Error('Cannot send a message without a reciever.')
    }

    // set reciever name
    doc.recieverName = Access.userFirstName(doc.recieverId) + ' ' +
        Access.userLastName(doc.recieverId)

    // start a thread inlcuding sender and reciever
    const userIds = [doc.senderId, doc.recieverId]
    Log.log(['debug', 'message', 'hook'],
        'before insert message new thread user ids:', userIds)
...
Messages.after.insert(function (userId, doc) {
  Log.log(['debug', 'message', 'hook'],
      'Schedule job for message waiting reminder email notification ', doc._id)
...
Jobs.register({
  'sendMessageWaitingNotification': function(messageId) {
    Log.log(['debug', 'message', 'job'],
      'Start sendMessageWaitingNotification job for message ', messageId)
    ...
    const thread = MessageThreads.findOne({_id: message.threadId},
          {fields: {userIds: 1}})
    ...
    // create thread url
    const threadUrl = Meteor.settings.ROOT_URL+'/messages/'+
        message.threadId;
