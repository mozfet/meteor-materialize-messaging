...
async function updateUnread (instance) {
  Log.log(['debug', 'message'], `updateUnread`)
...
// on created
Template.MessagingView.onCreated(() => {
  const instance = Template.instance()
  instance.state = {
    threads: new ReactiveDict()
  }
  instance.subscribe('messages.threads', () => updateUnread(instance))
...
// helpers
Template.MessagingView.helpers({
  threads() {
    const instance = Template.instance()
...
// on destroyed
Template.MessagingView.onDestroyed(() => {
  const instance = Template.instance()
